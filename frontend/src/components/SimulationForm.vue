<template>
  <Panel title="Параметры модели">
    ...
    <Accordion title="Биологические параметры">
      ...
      <div class="param-group">
        <NumberInput label="λ (приток CD4⁺)" v-model="localParams.biological.lambda" :step="1" :min="0" />
        <NumberInput label="r (пролиферация)" v-model="localParams.biological.r" :step="0.01" :min="0" :max="1" />
        <NumberInput label="T_max (ёмкость)" v-model="localParams.biological.T_max" :step="100" :min="0" />
        <NumberInput label="d_T (гибель CD4⁺)" v-model="localParams.biological.d_T" :step="0.01" :min="0" />
        <NumberInput label="β (скорость заражения)" v-model="localParams.biological.beta" :step="1e-6" :min="0" />
        <NumberInput label="ρ (доля латентных)" v-model="localParams.biological.rho" :step="0.05" :min="0" :max="1" />
        <NumberInput label="a (реактивация)" v-model="localParams.biological.a" :step="0.01" :min="0" />
        <NumberInput label="δ_L (гибель латентных)" v-model="localParams.biological.delta_L" :step="0.001" :min="0" />
        <NumberInput label="δ_I (гибель прод.)" v-model="localParams.biological.delta_I" :step="0.5" :min="0" />
        <NumberInput label="κ (уничт. CTL)" v-model="localParams.biological.kappa" :step="0.01" :min="0" />
      </div>
    </Accordion>
...
const defaultParams = reactive({
  ...
  therapy: {
    ...
    mode_prod: 'WITHOUT', epsilon0_prod: 0.8, gamma_prod: 0.01, start_day: 50 },
